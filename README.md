# Panda SMSPilot-Messenger-PHP-SDK
...
use Panda\SMSPilot\MessengerSDK\Delay;
...
```php
// Обязательные параметры: "API-ключ"
...
$massive->setTime(Delay::min(15))
    ...
    ->addRecipientList(['79995550012', '79995550013']);
...
try {
    $packet->addMessagetList([['Ваш друг уже с нами!', '79995550012'],
            ['Ваши друзья приняли наше предложение! А каким будет ваше решение?', '79995550013']]);
} catch (ClientException $e) {
    echo $e->getMessage();
}
...
$status->addMessage('23654545')
    ->addMessageList(['23654559', '23654665', '23654698']);
...
try {
    // Способ №2 (Продвинутый) (отправка посылки)
...
### Получение информации о балансе аккаунте (HTTP API v1)
...
$massive->addRecipient('79995550011')
    ...
    ->addRecipient('79995550013')

    // Списком
    // Обязательный параметр: "Номер получателя"
    ->addRecipientList(['79995550014', '79995550015'])
    ->addRecipientList(['79995550016', '79995550017', '79995550018', '79995550019']);
```
...
try {
    ...
    // Необязательные параметры: "Имя отправителя", "Время отправки"
    // Возможно использование функционала класса "Delay" в качестве параметра
    $packet->addMessageList([['Уже началось!', '79995550011', 'DEVELOPER', Delay::min(5)],
        ['..и никогда не закончится!', '79995550012', 'SMSPILOT', Delay::min(7)]])
    ->addMessageList([['Уже началось!', '79995550013', 'DEVELOPER', Delay::min(5)],
        ['..и никогда не закончится!', '79995550014', 'SMSPILOT', Delay::min(7)]]);
} catch (ClientException $e) {
    echo $e->getMessage();
}
```
...
$status->addMessage('23654545')
    ...
    // Списком
    // Обязательный параметр: "Номер сообщения"
    ->addMessageList(['23654548', '23654549'])
    ->addMessageList(['23654559', '23654665', '23654698']);
